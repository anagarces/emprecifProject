    ...
    <div style="max-width: 900px; margin: 0 auto; padding: 0 2rem;">
        ...
        @if(request()->filled('q'))
            ...
            @forelse($results as $empresa)
                @php
                    $latest = $empresa->accounts->first();
                    $facturacion = $latest->facturacion ?? $empresa->revenue ?? 0;
                    $resultado   = $latest->resultado_neto ?? $empresa->profit ?? 0;
                    $empleados   = $latest->empleados ?? $empresa->employees;
                ...
                <div style="background: white; padding: 2.5rem; border-radius: 15px; box-shadow: 0 10px 40px rgba(0, 0, 0, 0.06); border-left: 4px solid var(--primary); margin-bottom: 2rem;">
                    <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 1rem;">
                        <div>
                            <h3 style="font-size: 1.75rem; font-weight: 800; margin-bottom: 0.5rem;">
                                <a href="{{ route('company.show', $empresa) }}" style="color: var(--primary); text-decoration: none;">
                                    {{ $empresa->name }}
                                </a>
                            </h3>
                            <p style="color: var(--gray);">
                                NIF: {{ $empresa->cif }} • {{ $empresa->city }}, {{ $empresa->province }}
                            </p>
                        </div>
                    ...
                    {{-- Datos resumidos (solo usuarios autenticados, pero esta ruta ya está protegida) --}}
                    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 2rem; margin-top: 2rem;">
                        <div>
                            <p style="font-size: 0.875rem; color: var(--gray); margin-bottom: 0.25rem;">
                                Facturación {{ $latest->ejercicio ?? date('Y') - 1 }}
                            </p>
                            <p style="font-size: 1.5rem; font-weight: 900; color: var(--dark);">
                                {{ number_format($facturacion ?? 0, 0, ',', '.') }} €
                            </p>
                        </div>
                        <div>
                            <p style="font-size: 0.875rem; color: var(--gray); margin-bottom: 0.25rem;">
                            ...
                            <p style="font-size: 1.5rem; font-weight: 900; color: {{ $colorResultado }};">
                                {{ $signoResultado }}{{ number_format($resultado ?? 0, 0, ',', '.') }} €
                            </p>
                        </div>
                        <div>
                            <p style="font-size: 0.875rem; color: var(--gray); margin-bottom: 0.25rem;">
                    ...
                    <a href="{{ route('company.show', $empresa) }}" class="btn btn-primary" style="margin-top: 1.5rem;">
                        Ver Perfil Completo →
                    </a>
                </div>
            @empty
                <div style="text-align: center; padding: 4rem 2rem; background: #F9FAFB; border-radius: 15px;">
                    ...
                    <p style="color: var(--gray);">
                        No hay empresas que coincidan con tu búsqueda. Intenta con otros términos.
                    </p>
                </div>
            @endforelse
        @endif
