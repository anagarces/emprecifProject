...
<section>
    ...
    <div style="max-width: 900px; margin: 0 auto; padding: 0 2rem;">
        ...
        {{-- BÚSQUEDA --}}
        <form action="{{ auth()->check() ? route('company.search') : route('search') }}" 
              method="GET" class="search-box" style="margin-bottom: 4rem;">
            @csrf
            <input 
                type="text" 
                name="q" 
        ...
        @if(request()->has('q') && !empty(request('q')))
            ...
            @forelse($results ?? [] as $empresa)
                ...
                @php
                    $account = $empresa->accounts()->orderBy('ejercicio', 'desc')->first();
                @endphp
                ...
                <div style="background: white; padding: 2.5rem; border-radius: 15px; box-shadow: 0 10px 40px rgba(0, 0, 0, 0.06); border-left: 4px solid var(--primary); margin-bottom: 2rem;">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 1rem;">
                        <div>
                            <h3 style="font-size: 1.75rem; font-weight: 800; margin-bottom: 0.5rem;">
                                <a href="{{ route('company.show', $empresa->slug) }}" style="color: var(--primary); text-decoration: none;">
                                    {{ $empresa->name }}
                                </a>
                            </h3>
                    ...
                    {{-- FINANCIERO (solo usuarios logueados) --}}
                    @if(auth()->check())
                        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 2rem; margin-top: 2rem;">
                            ...
                            <div>
                                <p style="font-size: 0.875rem; color: var(--gray); margin-bottom: 0.25rem;">
                                    Facturación {{ $account->ejercicio ?? '—' }}
                                </p>
                                <p style="font-size: 1.5rem; font-weight: 900; color: var(--dark);">
                                    {{ $account ? number_format($account->facturacion, 0, ',', '.') : 'N/A' }} €
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                @php
                                    $resultado = $account->resultado_neto ?? null;
                                    $color = $resultado >= 0 ? '#065F46' : '#B91C1C';
                                    $signo = $resultado >= 0 ? '+' : '';
                                @endphp

                                <p style="font-size: 1.5rem; font-weight: 900; color: {{ $account ? $color : 'var(--dark)' }};">
                                    {{ $account ? $signo . number_format($resultado, 0, ',', '.') . ' €' : 'N/A' }}
                                </p>
                            </div>
                    ...
                    <a href="{{ route('company.show', $empresa->slug) }}" 
