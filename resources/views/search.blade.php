...
<section>
    ...
    <div style="max-width: 900px; margin: 0 auto; padding: 0 2rem;">
        <form action="{{ route('search') }}" method="GET" class="search-box" style="margin-bottom: 4rem;">
            @csrf
            <input 
                type="text" 
                name="q" 
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 1rem;">
                        <div>
                            <h3 style="font-size: 1.75rem; font-weight: 800; margin-bottom: 0.5rem;">
                                <a href="{{ route('empresas.show', $empresa['id']) }}" style="color: var(--primary); text-decoration: none;">{{ $empresa['nombre'] }}</a>
                            </h3>
                            <p style="color: var(--gray);">NIF: {{ $empresa['nif'] }} • {{ $empresa['localidad'] }}, {{ $empresa['provincia'] }}</p>
                        </div>
                        <span style="padding: 0.5rem 1.5rem; background: #D1FAE5; color: #065F46; border-radius: 50px; font-weight: 700; font-size: 0.875rem;">{{ $empresa['estado'] ?? 'ACTIVA' }}</span>
                    </div>
                    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 2rem; margin-top: 2rem;">
                        <div>
                            <p style="font-size: 0.875rem; color: var(--gray); margin-bottom: 0.25rem;">Facturación {{ date('Y')-1 }}</p>
                            <p style="font-size: 1.5rem; font-weight: 900; color: var(--dark);">{{ number_format($empresa['facturacion'] ?? 0, 0, ',', '.') }} €</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p style="font-size: 1.5rem; font-weight: 900; color: var(--dark);">{{ $empresa['empleados'] ?? 'N/A' }}</p>
                        </div>
                    </div>
                    <a href="{{ route('empresas.show', $empresa['id']) }}" class="btn btn-primary" style="margin-top: 1.5rem;">Ver Perfil Completo →</a>
                </div>
            @empty
